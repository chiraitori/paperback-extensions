...
export const KemonoFanboxInfo: SourceInfo = {
    version: '1.0.0',
...
export class KemonoFanbox extends Source {
    ...
    override async getHomePageSections(sectionCallback: (section: HomeSection) => void): Promise<void> {
        ...
        try {
            ...
            const posts: KemonoPost[] = data.posts ?? data ?? []
    ...
    override async getViewMoreItems(_homepageSectionId: string, metadata: any): Promise<PagedResults> {
        ...
        const items: PartialSourceManga[] = posts.map(post => {
            let thumbnail = ''
            if (post.file?.path && this.isImage(post.file.name)) thumbnail = this.getFileUrl(post.file.path)
            else {
                const imgAtt = (post.attachments || []).find(att => this.isImage(att.name))
                if (imgAtt) thumbnail = this.getFileUrl(imgAtt.path)
            }
    ...
    override async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
        ...
        try {
            ...
            const items: PartialSourceManga[] = filtered.map(creator => App.createPartialSourceManga({ mangaId: `${creator.id}/creator`, title: creator.name, image: `${KEMONO_BASE_URL}/static/kemono-logo.svg`, subtitle: `${creator.favorited} favorites` }))
            return App.createPagedResults({ results: items, metadata: { offset: offset + 20 } })
        } catch (e) { return App.createPagedResults({ results: [] }) }
    }
