...
class EHentai extends types_1.Source {
    constructor() {
        ...
        this.resolvedChapterCache = new Map();
        this.resolvingChapters = new Map();
    }
    ...
    async getChapterDetails(mangaId, chapterId) {
        ...
        if (cached)
            this.resolvedChapterCache.delete(cacheKey);
    ...
    async resolveChapterPages(mangaId, pageCount) {
        ...
        if (!firstImageUrl)
            throw new Error('E-Hentai first image page did not contain an image URL.');
    ...
    async getImagePageUrls(mangaId, pageCount) {
        ...
        firstPage('a[href*="?p="]').each((_linkIndex, element) => {
            ...
            if (match?.[1])
                lastPageIndex = Math.max(lastPageIndex, Number.parseInt(match[1], 10));
        });
        const remainingHtml = await Promise.all(Array.from({ length: Math.max(0, lastPageIndex) }, (_value, index) => this.get(`${galleryUrl}?p=${index + 1}`)));
    ...
    async getImageUrlFromApi(gid, imagePageUrl, showKey) {
        const match = /\/s\/([a-f0-9]+)\/\d+-(\d+)/i.exec(imagePageUrl);
        if (!match?.[1] || !match[2])
            throw new Error(`Invalid E-Hentai image page URL: ${imagePageUrl}`);
        ...
        if (!imageUrl)
            throw new Error('E-Hentai API response did not contain an image URL');
        return imageUrl;
    }
    async get(url) {
        const response = await this.requestManager.schedule(App.createRequest({
